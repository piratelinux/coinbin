$(document).ready(function() {
		...
		if(email.match(/[\s\w\d]+@[\s\w\d]+/g)){
			if($("#openPass").val().length>=10){
				if($("#openPass").val()==$("#openPassConfirm").val()){
					...
					$("#walletHistory").attr('href','https://tbtc.blockr.io/address/info/'+keys.address);
	...
	$("#walletLogout").click(function(){
		...
		$("#walletAddress").html("");
		$("#walletHistory").attr('href','https://tbtc.blockr.io/address/info/');
	...
	function listUnspentBlockrio_BitcoinMainnet(redeem){
		$.ajax ({
			type: "POST",
			url: "https://tbtc.blockr.io/api/v1/address/unspent/"+redeem.addr+"?unconfirmed=1",
			dataType: "json",
			error: function(data) {
			...
			success: function(data) {
				if((data.status && data.data) && data.status=='success'){
					$("#redeemFromAddress").removeClass('hidden').html(
						'<span class="glyphicon glyphicon-info-sign"></span> Retrieved unspent inputs from address <a href="https://tbtc.blockr.io/address/info/'+
